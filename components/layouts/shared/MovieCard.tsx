  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-[2/3]">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="space-y-2">
            ...
            <button className="w-full py-2 bg-linear-to-r from-red-500 to-purple-600 text-white text-xs font-semibold rounded-lg hover:shadow-lg transition-all duration-300 transform hover:scale-105">
              {item.priceType === "Premium" ? "Subscribe to Watch" : "Watch Now"}
            </button>
        ...
        {/* Rating Badge */}
        <div className="absolute top-2 right-1 px-2 py-1 bg-black/70 backdrop-blur-sm rounded-lg flex items-center gap-1">
          <Star
            className={`w-3 h-3 ${getRatingColor(item.averageRating)} fill-current`}
          />
          <span
            className={`text-xs font-bold ${getRatingColor(item.averageRating)}`}
          >
        ...
        <div className="absolute top-2 left-1 flex gap-1">
          {/* Media Type */}
          <div className="px-1 bg-black/70 backdrop-blur-sm rounded-md">
          ...
          >
            {item.priceType === "Premium" ? "Premium" : "Free"}
          </div>
        </div>
      </div>
